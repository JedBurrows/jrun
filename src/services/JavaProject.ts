...
export class ProjectRoot extends Context.Tag("ProjectRoot")<
  ProjectRoot,
  string
>() {}
...
export const JavaProjectLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const resolveClasspath = Effect.gen(function* () {
      ...
      const output = yield* Command.make("mvn", "dependency:build-classpath", "-q", "-DincludeScope=runtime", "-Dmdep.outputFile=/dev/stdout").pipe(
